...
def round_wrapper(filename, full_psi_file, full_imputed_psi_file, highly_variable_events, rank=None, min_group_size=5, dPSI=0.1, dPSI_p_val=0.05, min_differential_events=100, top_n_differential_events=150, conservation="stringent", strictness="easy", depletion_corr_threshold=0.3, write_files=True):
    ...
    if rank is None:
        k_results = determine_nmf_ranks(PSIdf=formatted_psi_file_imp_hve.transpose(), MADA=True, CORRINT=True, MOM=True, PCAFO=True, TLE=True, FS=True)
    ...
    basis_matrix, binarized_output_test, refined_binarized_output = run_nmf(imputed_psi_file=formatted_psi_file_imp_hve, rank=rank)
    ...
    refined_binarized_output_f = np.delete(refined_binarized_output, remove_clusters, axis=0)  # remove clusters (rows) with number of differential splicing events less than threshold (100)
